...
const PatientAnalysis: React.FC = () => {
  ...
  const list = analysedPatient.

  console.log(analysedPatient)
        ...
        <section className={style.LeftSection}>
          {!showModal && 
            <div className={style.patientName}>
              {analysedPatient.name}
            </div>}
          <AddImage patientId={analysedPatient.id} onChangeHandler={(previous) => setUpdate(previous+1)} />
          {showModal ? <ImageLoader setDisplay={setShowModel} img={modalImage}  /> :
            <div className={style.gridView}>
              {
                analysedPatient.proccessed_x_rays &&  analysedPatient.proccessed_x_rays.map(image => {
                  return <ImageCard
                    image={image.url}
                    type={image.type}
