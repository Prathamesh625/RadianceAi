...
const Dashboard: React.FC = () => {
  ...
  const get_doctor = async () => {
    ...
    const get_patient = await getPatientById(
      doctor.data.doctor.current_patient
    );
  ...
  useEffect(() => {
    const completed: number | any =
      appointments &&
      appointments.filter((value) => value.status === 'completed');

    setCompletedAppointments(
      Math.ceil((completed?.length / appointments?.length) * 100)
    );
  }, [appointments]);
        ...
        <section className={style.LeftSection}>
          ...
          <div className={style.cardFlex}>
            <Card
              ...
              content={patient ? patient.name : 'Empty'}
            />
